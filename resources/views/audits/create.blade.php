...
@section('title', 'Créer un audit - Audit Master')

@section('content')
<div class="main-container">
    <!-- FORMULAIRE -->
    <aside class="audit-form">
        <form action="{{ route('audits.store') }}" method="POST" id="audit-form">
            @csrf
            <div class="mb-4">
                <label class="fw-bold small text-uppercase">Client & Date</label>
                <input type="text" name="client_name" class="form-control mb-2" placeholder="Nom de l'entreprise" value="{{ old('client_name') }}" required>
                <input type="date" name="audit_date" class="form-control mb-2" value="{{ old('audit_date', date('Y-m-d')) }}" required>
            </div>
            ...
            <div id="form-sections">
                ...
                <div class="section-card category-item">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <input type="text" name="categories[0][title]" class="form-control form-control-sm fw-bold border-0 bg-transparent p-0" placeholder="Titre de la catégorie" value="Sécurité & Cyber" required>
                        <select name="categories[0][score]" class="form-select form-select-sm w-25">
                            @for($i = 1; $i <= 5; $i++)
                                <option value="{{ $i }}" {{ $i == 3 ? 'selected' : '' }}>{{ $i }}/5</option>
                            @endfor
                        </select>
                    </div>
                    <textarea name="categories[0][observations]" class="form-control form-control-sm mb-2" rows="2" placeholder="Observations..."></textarea>
                    <textarea name="categories[0][recommendations]" class="form-control form-control-sm" rows="2" placeholder="Recommandations..."></textarea>
                </div>
                
                <div class="section-card category-item">
                    <button type="button" class="delete-btn" onclick="removeCategory(this)">×</button>
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <input type="text" name="categories[1][title]" class="form-control form-control-sm fw-bold border-0 bg-transparent p-0" placeholder="Titre de la catégorie" value="Infrastructure IT" required>
                        <select name="categories[1][score]" class="form-select form-select-sm w-25">
                            @for($i = 1; $i <= 5; $i++)
                                <option value="{{ $i }}" {{ $i == 2 ? 'selected' : '' }}>{{ $i }}/5</option>
                            @endfor
                        </select>
                    </div>
                    <textarea name="categories[1][observations]" class="form-control form-control-sm mb-2" rows="2" placeholder="Observations..."></textarea>
                    <textarea name="categories[1][recommendations]" class="form-control form-control-sm" rows="2" placeholder="Recommandations..."></textarea>
                </div>
            </div>
            ...
            <div class="mt-4 p-3 border bg-light">
                ...
                <textarea name="conclusion" class="form-control" rows="5" placeholder="Votre synthèse globale...">{{ old('conclusion') }}</textarea>
            </div>
            
            <div class="mt-3 d-flex gap-2">
                <button type="submit" class="btn btn-nj w-50">Enregistrer</button>
...
@push('scripts')
<script>
    let categoryIndex = 2; // Commence à 2 car nous avons déjà 2 catégories
